          ...
          <div class="card z-index-0">
            ...
            <div class="card-body">
              <form role="form" @submit.prevent="onSubmit">
                <argon-input type="text" placeholder="Name" aria-label="Name" v-bind:value="name" @input="name = $event.target.value" />
                <argon-input type="email" placeholder="Email" aria-label="Email" v-bind:value="email" @input="email = $event.target.value" />
                <argon-input type="password" placeholder="Password" aria-label="Password"  v-bind:value="password" @input="password = $event.target.value"/>
                <ul class="d-flex align-item-centers list-tours">
                  <li v-for="(tour, index) in tours" :key="index" >
                    <argon-badge :color=" tour.selected ? 'success' : 'primary'" :class="{ 'selected': tour.selected }" @click="addBadge(tour)">{{ tour?.nameVN }}</argon-badge>
                  </li>
                </ul>
...
export default {
  ...
  data() {
    return {
      ...
      tours: [
        ...
        { id: 3, name: 'TOUR_EXPLORE', nameVN: 'khám phá', selected: true },
        { id: 4, name: 'TOUR_ADVENTURE', nameVN: 'phiêu lưu', selected: false },
        { id: 5, name: 'TOUR_CONVALESCENCE', nameVN: 'hồi phục sức khỏe', selected: false },
        { id: 6, name: 'TOUR_SIGHTSEEING', nameVN: 'tham quan', selected: false },
        { id: 7, name: 'TOUR_CAMPING', nameVN: 'cắm trại', selected: false }
      ]
    }
  },
  created() {
  ...
  methods: {
    ...
    addBadge: function(tour) {
      ...
      this.fieldTour = this.tours.filter(tour => tour.selected).map(tour => tour.name).join(',');
  },
  beforeUnmount() {
